...
module.exports = {
	...
	thoseWhoHaveNotFilledUpTheFormObx: function (api, users) {
		...
		return Rx.Observable.from(users).map(s => s.facebook_user_id).toArray().do(x => {
			console.log("# ===================================================");
			...
			return Rx.Observable.from(datasource.formdevUsers).filter(s => {
				...
				if (user.message_on) user.delay = moment(user.message_on, "YYYY-MM-DD HH:mm:ss");
	...
	thoseWhoHaveNoGdocsObx: function (api, users, threads) {
		...
		var facisWithoutGdocsObx = Rx.Observable.from(users).filter(s => s.gdocs == '').map(s => {
		...
		return facisWithoutGdocsObx.do(user => console.log("User " + user.nickname + " has not yet submitted Gdocs."))
		...
		.flatMap(user => {
			...
			return this.remind(api, message, user.threadID, user.delay).do(user => {
				console.log("Sent!\n");
			});
		});
	}
