...
const DataSource = {
  ...
  */
	...
	_persistAppState: function (api) {
		...
		fs.writeFileSync('./static/appstate.json', JSON.stringify(api.getAppState(), null, 2));
		if (this.verbose) {
  ...
  */
	login: function (options) {
		...
		return this._loginProcess(options).pipe(catchError(error => {
			...
			if (this.verbose) logger.verbose("Successfully logged in to Facebook Messenger.");
		}));
	},
